...
class CountryStatistics(models.Model):
    ...
    country_abbreviation = models.TextField(
        verbose_name="country_abbreviation")
    total_cases = models.TextField(verbose_name="total_cases")
...
class GeneralStatisticsSerializer(serializers.HyperlinkedModelSerializer):
    class Meta:
        model = GeneralStatistics
        fields = ['id', 'total_cases', 'recovery_cases', 'death_cases',
                  'last_update', 'currently_infected', 'last_fetched_date']
